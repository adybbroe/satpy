...
class LimbCoolingCorrection(CompositeBase):
    """Performing the limb-cooling correction of IR bands."""

    def get_angles(self, tb_):
        """Get the satellite view angles using Pyorbital."""
...
def get_limb_cooling_correction(sat_zenith, name, tb11_data):
    ...
    idx_w = da.clip((wv_ / 0.5 - 1).astype('int'), 0, 14)
    idx_t = da.clip(((tb11_data - 230) / 10.0).astype('int'), 0, 7)
